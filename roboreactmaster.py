...
class Action_control(object): 
           ...
           def I2C_DC_motordriver(self,i2c_address,gpiol,gpior):
                  pass 
           def I2C_servo_motor(self,name_servo,freq,angle,pins):
                try:  
                    #pca = PCA9685(i2c)
                    #pca.frequency = 50
...
class Visual_Cam_optic(object):  # Calling the camera and optic devices input for image processing publish and subscriber on one platform to be easy to movidy with one library 
           ...
           def Camera_raw(self,cam_num,Buffers,portdata,ip_number): # Getting the raw image of the camera
                ...
                exec("execport = "+str(portdata)) # Getting the portdata from the list to getting the camera data
           ...
           def Camera_QR(cam_num,frame):  # Getting the raw camera image 
                  ret, frame = cv2.VideoCapture.read(cam_num)
                  if ret:
...
#Create_node_pub("Servo_2",message,"127.0.0.1",5082) 
...
if __name__=="__main__":
          ...
          p4 = threading.Thread(target=mcu2)
